...
def test_internal_package_import_emits_deprecation():
    """Importing from aegis._internal emits DeprecationWarning."""
    import aegis._internal as mod
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert "aegis._internal" in str(w[0].message)
        assert "from aegis import" in str(w[0].message)
        assert callable(func)


def test_public_import_no_deprecation():
    """Importing from aegis does NOT emit DeprecationWarning."""
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        from aegis import enforce_invocation  # noqa: F401
        deprecation_warnings = [
            x for x in w
            if issubclass(x.category, DeprecationWarning)
            and "aegis._internal" in str(x.message)
        ]
