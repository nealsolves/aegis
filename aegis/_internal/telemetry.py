...
try:
    ...
    _tracer = trace.get_tracer("aegis.enforcement")
except ImportError:
    pass
...
def enforcement_span(
    ...
    :param name: Span name (e.g. "aegis.enforce_invocation")
...
def record_gate_event(
    ...
    *,
    ...
    try:
        span.set_attribute(f"aegis.gate.{gate_id}.status", status)
        if details:
            for k, v in details.items():
                if isinstance(v, (str, int, float, bool)):
                    span.set_attribute(f"aegis.gate.{gate_id}.{k}", v)
...
def record_enforcement_result(
    ...
    *,
    ...
    try:
        span.set_attribute("aegis.enforcement.result", result)
        if policy_file:
            span.set_attribute("aegis.enforcement.policy_file", policy_file)
        if role:
            span.set_attribute("aegis.enforcement.role", role)
        if risk_score is not None:
            span.set_attribute("aegis.enforcement.risk_score", risk_score)
        if enforcement_mode is not None:
            span.set_attribute("aegis.enforcement_mode", enforcement_mode)
    except Exception:  # noqa: BLE001
