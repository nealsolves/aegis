"""
Policy CLI for Aegis governance SDK.

Provides:
- ``aegis policy lint`` — YAML syntax and schema validation
- ``aegis policy validate`` — full semantic validation including extends
- ``aegis policy init`` — generate a governance policy scaffold
- ``aegis compliance export`` — compliance export of invocation audit artifacts
- ``aegis workflow init`` — generate a workflow starter scaffold
- ``aegis workflow lint`` — statically lint governance targets
- ``aegis workflow doctor`` — runtime/evidence diagnostics
- ``aegis workflow trace`` — reconstruct workflow timelines from JSONL artifact files
- ``aegis workflow export`` — export governed workflow evidence (operator or audit mode)
"""
    ...
    try:
        with open(input_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                # invocation audit records. Skip them silently — aegis workflow
                # export handles them.
...
def _cmd_workflow_trace(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        from aegis._internal.audit import checksum as _checksum
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="aegis",
        description="Aegis Governance SDK CLI",
    )
    ...
    # aegis policy init --profile <profile> [--output <path>] [--role <role>]
    ...
    # aegis compliance export --input <file> [--output <file>]
    ...
    # aegis workflow init --profile <profile> [--output-dir <dir>] [--role <role>]
    ...
    # aegis workflow lint <targets...> [--kind auto|policy|starter_dir|workflow_artifact] [--json]
