...
> Active file: `docs/plans/AEGIS V0.9.0 IMPLEMENTATION_PLAN.md`.
...
- Make the primary release story brutally simple: a normal Python team can drop AEGIS into a real local workflow the week of release, get a first success quickly, hit a real failure, understand it, fix it, and succeed again.
- Keep the architecture honest:
  - AEGIS remains an SDK, not a hosted orchestrator, transport, or runtime platform.
...
- The default user story is a simple local workflow in a host-owned Python app using ordinary `AEGIS.open_session(...)` session semantics.
...
`v0.9.0` is the beta release where AEGIS stops being merely a set of workflow-capable primitives and becomes a practical workflow-governance SDK that app teams can adopt quickly.
...
5. Open a session through `AEGIS.open_session(...)`.
...
- Implement:
  ...
  - Mark competing artifacts as `superseded` or `historical input`, including:
    ...
    - `docs/plans/AEGIS v0.9.0 Practical Implementation Plan.docx`
    - `docs/plans/AEGIS v0.9.0 to v1.0.0 PLAN.md`
    - `docs/plans/AEGIS v1.0 Development Plan.pdf`
...
- Implement:
  ...
  - Freeze:
    ...
    - `AEGIS.open_session(...)` instance-scoped adoption
...
- Exit:
  - AEGIS can govern a simple local workflow end to end without adapters, hidden orchestration, or internal APIs.
...
- Exit:
  - operators can inspect, trace, and export governed workflows without turning AEGIS into a monitoring platform.
...
- The SDK boundary doc must say clearly what the host owns and what AEGIS owns.
...
- One workflow trace view.
- One audit and export visibility flow.
- No fake backend behavior. Demo behavior must reflect real host-owned execution and real AEGIS governance decisions.
...
- Optionally wrap one existing path in `AEGIS.open_session(...)` and keep host orchestration unchanged.
