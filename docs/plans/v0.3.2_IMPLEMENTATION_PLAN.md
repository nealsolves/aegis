...
flowchart TD
  ...
  S2 --> S3
  S3 --> S4[Step 4 Public API Exports + AEGIS Instance Split Methods]
...
- `aegis/_internal/errors.py` (only if new typed error for invalid pre-call result is required)
...
- `tests/test_split_enforcement_aegis_instance.py`
...
- `aegis/enforcement.py`
- `aegis/__init__.py`
- `tests/test_public_api.py`

Public API changes:

- Export split functions and `PreCallResult` from top-level `aegis` and `aegis.enforcement`.
