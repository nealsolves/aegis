...
> “AEGIS 0.9.0 should be an easy thing app teams can drop into a real workflow the week of release.”

This plan exists to make that statement true without weakening architecture.

- AEGIS remains an SDK, not a hosted orchestrator, runtime, transport, or model-serving platform.
...
- AEGIS continues to own policy loading, ordered governance checks, workflow constraints, evidence correlation, optional adapter normalization, and audit artifacts.
...
Bedrock, A2A, and the OpenAI Agents SDK stay in scope, but they are advanced optional tracks. They are not the default story, they are not the default dependency chain, and they are not the bar for first success.
...
4. Drop AEGIS into a simple local `2`-step or `3`-step workflow in a host-owned app.
...
- First success must not require Bedrock, A2A, or the OpenAI Agents SDK.
...
| --- | --- | --- |
| Fresh install -> minimal workflow scaffold -> PASS | A clean environment installs base `aegis`, runs `aegis workflow init`, selects `minimal`, drops the starter into a local host app, opens a session through `AEGIS.open_session(...)`, and completes a passing workflow with correlated evidence. | PASS |
...
| First adopter gets value without optional adapters | The local workflow path is useful on its own and the adopter can complete the first governed workflow without Bedrock, A2A, or OpenAI Agents SDK dependencies. | PASS |
...
- AEGIS owns policy loading, ordered governance checks, workflow constraints, evidence correlation, optional adapter normalization, and audit artifacts.
...
- AEGIS governs only host-visible handoff and evidence boundaries, not hidden Bedrock-internal orchestration.
...
Adoption data in this plan means lightweight release evidence gathered from clean-room runs, beta feedback worksheets, issue triage, and explicit release review notes. It is not product telemetry and it must not turn AEGIS into a hosted data-collection platform.
...
- count of journeys that succeeded without Bedrock, A2A, or the OpenAI Agents SDK
...
- Implement:
  ...
  - freeze `AEGIS.open_session(...)` as the workflow entrypoint
  - freeze invocation-artifact and workflow-artifact separation
  - add fail-closed contract tests for Bedrock, A2A, and OpenAI Agents SDK boundaries
- Tests:
  - contract doc lint
...
- Implement:
  ...
  - add `AEGIS.open_session(...)` as the public workflow entrypoint
...
- Exit:
  - AEGIS can govern a simple local workflow end to end without adapters or hidden orchestration
...
- Exit:
  - operators can inspect and export governed workflows without turning AEGIS into a monitoring platform
...
- Branches:
  ...
  - `feat/v0.9-10-openai-agents-adapter`
...
- Implement OpenAI Agents SDK:
  - add `OpenAIAgentsAdapter` for host-owned `Runner` calls built on the
    official `openai-agents` runtime
  - add additive `openai_agents` protocol constraints and governed participant
    binding for root agents and trace-visible identities
  - normalize run results, optional tracing summaries, interruptions, and
    observed handoffs into workflow-step metadata without persisting raw payloads
  - add a narrow dynamic tool-governance seam for adapter-managed function tools
    and `Agent.as_tool(...)` wrappers
  - reject unsupported governed surfaces explicitly, including runtime features
    AEGIS cannot authorize before execution
- Tests:
  ...
  - OpenAI Agents binding and trace-required tests
  - OpenAI Agents wrapped-tool governance tests
...
- Bedrock, A2A, and the OpenAI Agents SDK remain in scope as advanced optional tracks.
...
- AEGIS normalizes host-visible evidence only.
...
- the public API boundary document must state clearly what the host owns and what AEGIS owns
...
- evidence showing whether adopters succeeded without Bedrock, A2A, or the OpenAI Agents SDK
...
- they do not need Bedrock, A2A, or the OpenAI Agents SDK for first success
