...
- Scope reviewed: `develop...feat/v0.9-08-engine-hardening` with focus on workflow runtime enforcement, restrictive composition, validator-hook behavior, workflow-doctor mappings, schema changes, and the targeted PR-08 tests. I also verified that `aegis/schemas/policy_dsl.schema.json` and `schemas/policy_dsl.schema.json` are byte-identical.
- Commands run:
  - `git status -sb`
...
- Location: `aegis/_internal/validator_hook.py:185`, `aegis/_internal/session.py:881`, `tests/test_validator_hook.py:445`
...
- Location: `aegis/_internal/session.py:343`, `aegis/_internal/session.py:280`
...
- Location: `aegis/_internal/policy_loader.py:379`
...
- No public-surface freeze test inspects the exported `aegis.GovernanceSession` constructor for `validator_hooks`.
