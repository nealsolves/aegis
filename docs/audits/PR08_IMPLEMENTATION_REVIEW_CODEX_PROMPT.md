# Codex Prompt: PR-08 Implementation Review

Use the prompt below as-is with Codex when you want a findings-first review of
the PR-08 implementation, with the review written to `docs/audits` as Markdown.
...
- Use this filename:
  YYYY-MM-DD-pr08-implementation-code-review-codex.md
...
- Prioritize bugs, fail-open behavior, governance regressions, race conditions, public-contract drift, and missing tests
...
- Capture the exact HEAD SHA for `feat/v0.9-08-engine-hardening`
...
- Extract actual acceptance criteria, fail-closed expectations, and non-goals before judging implementation

Phase 3: Inspect the implementation
- Review runtime enforcement, policy composition, workflow error taxonomy, validator-hook behavior, artifact schema additions, and doctor mapping
- Confirm whether the implementation is deterministic, fail-closed, and coherent across runtime, schemas, tests, and contract-freeze checks
...
- Verify rejected pre-calls do not advance counters, sequence position, handoff state, escalation state, or transition state
- Verify `max_steps` is enforced pre-call and step counters increment only after all checks pass
- Verify `max_total_tool_calls` is checked pre-call and reconciled post-call without double counting or silent drift
...
- Verify participant, role, protocol, and handoff checks fail closed when declared
- Verify escalation pauses before failure, creates evidence, and does not immediately re-fire after a valid resume
- Verify pending or denied checkpoints block `complete()`
- Verify wrong `approval_id` cannot silently approve the wrong checkpoint
- Verify artifact finalization preserves evidence on pass, fail, pause, and cancel paths
...
- Verify hook evidence records enough provenance to audit later
...
- Explicitly verify `WORKFLOW_STEP_BUDGET_EXCEEDED` and `WORKFLOW_HOOK_DENIED` do not leak into frozen public reason-code lists where they are not supposed to
- Verify workflow artifact schema additions are emitted consistently:
  - `approval_checkpoints`
  - `validator_hook_evidence`

5. Test quality
- Review whether the tests prove behavior rather than only matching implementation details
