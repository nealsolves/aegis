...
- PR-08 hardening code is strong, but the shipped-vs-planned story for `ValidatorHook` and related surfaces is not coherent, the hook path is effectively dead outside tests, and several new workflow exceptions raised from public methods are not catchable from `aegis` or `aegis.errors`.

## Review Scope and Method

- Audited local repo state at `/Users/neal/Documents/_Shenanigans/_myProjects/aegis` on branch `develop`; `git status --short` showed unrelated untracked `docs/plans/v0.9.0_PR-07_BETA_PROOF_PLAN.md`, which I did not use as source of truth.
...
| PR-02 | contract freeze | PARTIAL | Core session and artifact contract is implemented, but the freeze is not coherent across docs | `aegis/_internal/enforcement.py:1587-1608,2385-2403`; `aegis/_internal/session.py:64-79,474-489,834-839` | Resolve public/planned contract drift |
| PR-03 | golden-path contract | DRIFTED | Public-import rules are enforced, but CLI contract is not frozen coherently | Plan `385-392`; HLD `667-675`; `CLAUDE.md:117-123`; actual CLI `aegis/_internal/cli.py:453-529` | Decide whether trace/export are beta or PR-09 and align docs/tests |
| PR-04 | minimal session flow | COMPLETE | Real host-owned local governed workflow path exists and passes | `aegis/_internal/session.py:451-501`; proof harness `scripts/validate_v090_beta_proof.py:213-260` | No blocking PR-04-specific gap found |
| PR-05 | starters and migration | COMPLETE | Scaffolds, presets, policy init, and migration path are real and public-import clean | `aegis/_internal/cli.py:394-415,460-482`; starter/migration smoke tests; public import scan | Stale docs and truth-check drift around this work |
| PR-06 | doctor and lint | PARTIAL | Tools exist and codes are stable, but semantic coverage falls short of contract | `aegis/_internal/workflow_lint.py:126-321`; `aegis/_internal/workflow_doctor.py:49-112,340-389` | Add real transition/budget/binding diagnostics or narrow claims |
...
| PR-08 | engine hardening | PARTIAL | Hardening code is real, but hook registration is not productized and public exception/export boundaries are incomplete | `aegis/_internal/policy_loader.py:299-531`; `aegis/_internal/session.py:168-171,680-919`; `tests/test_validator_hook.py:15-43,296-306`; `aegis/errors.py:1-47` | Decide whether hooks are truly shipped/internal-only, wire or remove dead hook path, and export or normalize public workflow exceptions |
...
- Public-import boundary enforcement is real: `rg "aegis\\._internal" README.md PROJECT.md docs examples demo-app-react demo-app-api` found only engineering-plan/audit references, not maintained public code, and `tests/test_v090_contract_freeze.py:178-192` also checks for leakage.
...
- `GovernanceSession.finalize()` emits one workflow artifact with correlated invocation checksums in `aegis/_internal/session.py:451-501`.
- Additive correlation fields are injected into each step invocation context in `aegis/_internal/session.py:834-839`.
...
- Ship `aegis workflow init` and `aegis policy init`.
...
- `aegis policy init` is registered in `aegis/_internal/cli.py:394-415`; `aegis workflow init` is registered in `aegis/_internal/cli.py:460-482`.
...
- `aegis workflow lint` should cover schema, transitions, bindings, budgets, starter integrity, and public-import safety.
- `aegis workflow doctor` should cover runtime/evidence diagnosis.
...
- CLI surface exists in `aegis/_internal/cli.py:484-529`, with `--kind` and `--json` support for both commands.
- Stable next-action registry is centralized in `aegis/_internal/workflow_doctor.py:49-112`.
- Policy lint covers file readability, YAML parse, root type, JSON schema validation, date inversion, duplicate tool names, and output-schema validity in `aegis/_internal/workflow_lint.py:126-233`.
- Starter lint only checks required files, non-empty files, nested policy lint, AST parse, and `_internal` imports in `aegis/_internal/workflow_lint.py:245-321`.
- Starter doctor is pattern-based: it infers approval and source-required advisories by regex-scanning `workflow_example.py` in `aegis/_internal/workflow_doctor.py:326-391`.
...
- Restrictive composition is extensive in `aegis/_internal/policy_loader.py:299-531`: max steps, total tool calls, participants, roles, protocols, manifest refs, sequence, allowed transitions, handoffs, escalation, and protocol constraints all reject widening merges.
...
- `pyproject.toml:8-25` and `aegis/__init__.py:100` still identify the package as `0.3.3` alpha, while multiple docs speak about a `v0.9.0` beta line available from source. That may be intentional pre-release behavior, but it is not explained cleanly in one place.
...
How to verify: In a fresh generated regulated starter, remove `source_ids` in `workflow_example.py`, run it and observe failure, run `aegis workflow doctor` against that same directory, apply the documented fix to that same file, rerun the same directory to `COMPLETED`, and ensure demo/tests/harness all use the same path.
Blocks beta readiness: YES
...
Likely files to change: `aegis/_internal/workflow_lint.py`, `aegis/_internal/workflow_doctor.py`, `tests/test_workflow_lint.py`, `tests/test_workflow_doctor.py`, `docs/reference/TROUBLESHOOTING.md`
...
PR: PR-08
Why it matters: App teams should be able to catch public runtime failures without importing `aegis._internal`.
Likely files to change: `aegis/errors.py`, `aegis/__init__.py`, `aegis/_internal/session.py`, `tests/test_v090_contract_freeze.py`, new public-surface tests
How to verify: `python -c "import aegis, aegis.errors as e; print(hasattr(aegis, 'WorkflowSequenceViolationError'), hasattr(e, 'WorkflowSequenceViolationError'))"`; add one black-box test per exported or normalized public failure family.
Blocks beta readiness: YES
...
Likely files to change: `pyproject.toml`, `aegis/__init__.py`, `CHANGELOG.md`, `README.md`
How to verify: `python -c "import aegis; print(aegis.__version__)"`; inspect `pip show` metadata from an editable install; confirm docs explain whether the mismatch is intentional pre-release state or an oversight.
...
Likely files to change: `aegis/_internal/session.py`, `aegis/_internal/enforcement.py`, `tests/test_session_core.py`, new workflow-session failure-path tests
...
- After the docs/contract/proof fixes land, rerun the exact release gate set and only reassess readiness once `python -m pytest`, `flake8 aegis`, `python scripts/check_doc_parity.py`, `pytest demo-app-api/tests -q`, `npm --prefix demo-app-react test`, and `npm --prefix demo-app-react run build` are all green.
...
- `rg "aegis\\._internal" README.md PROJECT.md docs examples demo-app-react demo-app-api` -> no maintained public example/demo imports from `aegis._internal`
...
- `python -m aegis workflow --help` -> only `init`, `lint`, and `doctor` subcommands are present
- `python -m aegis workflow trace --help` -> invalid subcommand
- `python -m aegis workflow export --help` -> invalid subcommand
- `python -m aegis policy init --help`
- `python -m aegis workflow lint --help`
- `python -m aegis workflow doctor --help`
...
- `aegis/_internal/workflow_init.py`
- `aegis/_internal/policy_init.py`
