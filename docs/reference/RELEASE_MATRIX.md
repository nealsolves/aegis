...
| Current documentation/demo baseline | `v0.9.0 beta source` | PR #18 merged at `origin/develop` merge commit `fdf3649`; not on `main`. | Source checkout only. | Candidate runtime plus reconciled maintained docs, regenerated architecture assets, and eleven-lab demo source. | PyPI publication, `main` cutover, and a verified public beta deployment. | Same candidate adapter contract as the package baseline. | Current `develop` source truth. | PR #18 validation passed; this row does not claim the follow-up deployment is live. | No published package or deployment artifact is implied. | Feature-branch delivery and deployment remain separately policy-gated. | GitHub Pages and Render are configured by the follow-up change; live URLs are recorded only after smoke tests pass. |
