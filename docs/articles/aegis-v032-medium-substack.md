# AEGIS v0.3.2: Governance at the Invocation Boundary

*Release-accurate to AEGIS v0.3.2, shipped 2026-04-05. All code and claims verified against the public repository at [github.com/nealsolves/aegis](https://github.com/nealsolves/aegis).*
...
AEGIS treats governance as infrastructure. Every invocation attempt is evaluated against a declared policy, checked through a fixed gate sequence, and produces a structured audit artifact. Pass or fail. Every time.

---

## Part 2: What AEGIS Is

AEGIS is a provider-agnostic Python SDK. It does not make model calls. It does not replace your orchestration framework. It does not store artifacts on your behalf.
...
AEGIS operates between application logic and model provider. The host assembles an invocation. AEGIS loads the policy, runs the governance pipeline, and emits an audit artifact. The host then decides what to do with the model call and the result.
...
This is the fixed gate sequence AEGIS runs on every invocation. Gate ordering is a design invariant. It cannot be changed through configuration.
...
Domain-specific controls belong to the host. AEGIS supports custom gates at four fixed insertion points: `pre_authorization`, `post_authorization`, `pre_output`, `post_output`.
...
The `AEGIS` class is the production entry point. It owns sink, signer, gates, and policy cache with no global mutable state. Configuration is immutable after construction. `AEGIS.enforce()` is thread-safe.
...
Prior to v0.3.2, all enforcement happened in a single call. The model had to run before AEGIS could evaluate outputs. That meant token spend occurred before authorization was confirmed.
...
Below is the full capability surface of AEGIS across its release history. Each layer was added without breaking the layer beneath it.
...
│  AEGIS class / Typed preconditions / Exception sanitization   │
...
The fastest way to understand AEGIS governance in practice is the interactive demo, deployed at [nealsolves.github.io/aegis](https://nealsolves.github.io/aegis).
...
The governance pattern in AEGIS is not domain-specific. The contracts change. The architecture does not.
...
**Clinical decision support.** The host owns the final action. AEGIS governs what the model is permitted to propose and validates that the proposal matches the declared contract. A clinician sees a governed output, not a raw model response. The audit trail survives internal review and external investigation.
...
AEGIS takes a hard line on where AI governance belongs.
...
For enterprises, the value is concrete. AEGIS does not ask leaders to trust that governance happened. It gives them a way to enforce it, prove it, and inspect it as part of the system that actually made the AI call.
...
*All feature references in this article are verified against AEGIS v0.3.2 CHANGELOG and architecture documentation. No claims are aspirational.*
