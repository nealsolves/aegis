# AEGIS v0.3.2: Put Governance Where the Model Call Actually Happens
...
Yes. This is a correct way to state AEGIS’s governing principle, and it aligns with the project docs:
...
That wording is stronger and more accurate than a looser “AI should be governed responsibly” statement because it makes five implementation-level claims. In AEGIS, those are not aspirations. They are design constraints.

If you want the crispest version for a Medium article, I would use this sentence:

> **AEGIS’s governing principle is simple: no AI-influenced behavior is valid unless it is explicitly specified, deterministically enforceable, externally observable, replayable and auditable, and governed independently of any specific model or provider.**
...
AEGIS is built on a narrower and more practical claim:
...
If a model is called by the wrong role, if required context is missing, if a tool exceeds its policy cap, or if the output breaks the declared schema, AEGIS is designed to catch that in the enforcement path and emit an audit artifact for the result.
...
AEGIS is a **provider-agnostic Python SDK** for governance at the AI invocation boundary.
...
Think of AEGIS as the layer between your application and the model provider:

**Application → AEGIS governance → Model provider**

The application owns the business workflow.
AEGIS owns the enforcement boundary.
...
In v0.3.2, AEGIS runs enforcement through a fixed gate sequence. The important point is not memorizing every internal step. The important point is that **gate ordering is fixed and enforcement is fail-closed**.

In practical terms, AEGIS checks things like:
...
And on every path, AEGIS produces evidence.
...
This is where AEGIS becomes more interesting than a validator.

Failure is not just an exception. It is also an artifact.

AEGIS raises typed errors, and the failure path carries structured evidence with it. That matters because enterprises do not just need to know that something broke. They need to know:
...
In other words, **AEGIS treats failure as a first-class governance event**.
...
It means AEGIS can now block unauthorized invocations **before token spend**.
...
AEGIS’s `PreCallResult` is not just a convenience object. It is the evidence bridge between Phase A and Phase B.
...
AEGIS answers that with a signed, one-time-use handoff.
...
AEGIS artifacts are designed to carry:
...
AEGIS does not pretend to be a universal AI safety layer. It does not promise factual correctness. It does not replace application authorization. It does not eliminate the need for human review in high-stakes settings.
...
## The deeper idea behind AEGIS

AEGIS is built on a simple but important inversion:

Most systems treat the model as the center and governance as an accessory.

AEGIS treats governance as part of the execution contract and the model as a pluggable probabilistic component inside that boundary.
...
AEGIS is not interesting because it says governance matters.
