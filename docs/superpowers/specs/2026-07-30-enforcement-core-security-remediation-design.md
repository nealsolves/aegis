...
The implementation is divided into five internal units with one responsibility
each:
...
2. `RestrictionComparator` proves that a candidate is not weaker than its
   authority envelope.
3. `OutcomeNormalizer` maps all gates, hooks, risk evaluations, and internal
   failures into a closed authorization result.
4. `OperationRegistry` owns atomic single-use split-operation state.
5. `EvidenceFinalizer` owns checksum, signature, and emission ordering.
...
so lint warnings alone are insufficient. Typed preconditions and output schemas
use the same compiler-enforced safe subset:

- maximum pattern length is `256` characters;
- maximum candidate value is `16_384` UTF-8 bytes;
- literals, escaped literals, anchors, character classes, concatenation, and
  bounded repetitions are supported;
- a bounded repetition upper limit may not exceed `256`;
- unbounded repetition, grouping, alternation, lookaround, backreferences,
  conditionals, and nested quantifiers are rejected.

The supported example `^APPROVED-[0-9]{6}$` remains valid. Patterns are compiled
once. Runtime never evaluates an uncompiled policy string. Workflow lint reports
the exact policy/schema path and rejected construct.
...
The content-checksum payload is canonical JSON of the final artifact excluding
only:
...
profile fields. Legacy workflow artifacts remain readable as unsigned,
unanchored evidence but cannot satisfy the new verified-workflow contract.
...
- Policy compilation failure blocks before Phase A and produces finalized FAIL
  evidence when enough invocation identity exists to do so safely.
...
- Sink failure behavior remains an explicit instance setting, but every sink
  attempt receives the same already-finalized object.
...
- Exercise the safe pattern grammar, length bounds, candidate bounds, and
  rejected backtracking constructs in typed preconditions and nested output
  schemas.
...
#### B1. Strict checksum and verification result
...
- architectural call-site fitness test;
...
2. A2 closed gate, hook, and risk outcomes;
3. A3 process-affine operation registry;
4. B1 strict checksum and typed verification;
5. B2 central finalizer and workflow signing;
...
- the architecture fitness tests pass.
