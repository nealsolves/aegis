# The Three C's of AEGIS: Contract, Control, Check
...
Current public beta: `aegis-ai-governance==0.9.0b1`, released from `main`. It
extends the framework from invocation governance to
...
This is the foundation of AEGIS's design principle: governance is data, not
application code.
...
For AEGIS's core governance gates, the answer is fail-closed.
...
This is the most current expression of Check in AEGIS: evidence must be
structurally hard to forge, not just convenient to emit.

### PASS and FAIL are both evidence

AEGIS treats failed invocations as first-class audit events. An unauthorized
...
AEGIS governs invocations, not model vendors. The Three C's apply the same way
...
The Three C's are the governance lens over the broader AEGIS runtime:
