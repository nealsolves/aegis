...
AEGIS remains an SDK. It is not a hosted platform, not an orchestrator, not a
protocol runtime, and not a replacement for the host application's business
logic.

One-line rule: the host performs actions; AEGIS governs whether they are allowed
and emits evidence.
...
|                                                                                |
|  +-------------------------+      +-----------------------------------------+  |
|  | Optional Adapters       | ---> | AEGIS Workflow Governance Layer          |  |
...
|                                                       v                        |
|                                   +-----------------------------------------+  |
|                                   | AEGIS Invocation Governance Kernel       |  |
...
|                        host-owned; AEGIS governs and evidences them            |
|                                                                                |
...
AEGIS 1.0.0 is designed to provide:
...
AEGIS 1.0.0 preserves the invocation kernel and extends it with workflow
...
| **Boundary discipline** | The host owns orchestration and execution; AEGIS owns governance and evidence. |
...
AEGIS sits between host intent and host execution. The host decides what to try.
AEGIS decides whether that step is policy-valid and what evidence must be
emitted.

### 4.1 Ownership Boundary

| Host Application Owns | AEGIS Owns |
| --------------------- | --------- |
...
host prepares one invocation attempt
  -> AEGIS validates policy + role + conditions + tools + output contract
  -> AEGIS emits one invocation artifact

Workflow Boundary
-----------------
host coordinates many invocation attempts and handoffs
  -> AEGIS tracks governance session state
  -> AEGIS enforces sequencing, participant, protocol, budget, and escalation rules
  -> AEGIS correlates invocation artifacts into workflow evidence
```

Workflow governance is stateful inside AEGIS, but only for governance state.
AEGIS tracks session identifiers, step progress, approvals, budgets, and
...
| ------- | ------- | ---------------- | ------------ |
| `Policy` | Declarative contract for invocation and workflow governance | AEGIS public DSL | application orchestration |
| `Invocation` | Structured description of one governed invocation attempt | host supplies, AEGIS validates | provider runtime |
...
The invocation kernel remains the stable core of AEGIS.
...
Workflow governance is the architectural center of AEGIS 1.0.0.
...
1. **registration**: the host declares the intended step, participant, and
   protocol context
2. **authorization**: AEGIS validates transitions, participant identity,
   protocol constraints, budgets, and escalation rules
3. **completion**: the host provides invocation output or normalized external
   evidence, and AEGIS binds the step to invocation evidence
...
AEGIS 1.0.0 distinguishes between invocation evidence and workflow evidence.
...
Adapters normalize external workflow or protocol evidence into AEGIS-native
...
AEGIS remains extensible, but every extension point is constrained by the same
fail-closed model.
...
AEGIS treats governance as a trust-boundary problem. The SDK must be explicit
...
host-supplied input.
AEGIS never treats raw provider or transport state as trusted without explicit
...
| ------------------ | ---------- |
| `AEGIS` | instance-scoped entry point for invocation governance |
...
AEGIS 1.0.0 targets the following release properties:
