# AIGC Architectural Redesign + Execution Roadmap
...
AIGC is a deterministic, fail-closed policy enforcement engine for AI and agent invocations that produces tamper-evident audit artifacts
...
2. **Stop using module-level global state for sinks/config.** All mutable state moves to an `AIGC` instance object. Module-level `set_audit_sink()` becomes a deprecated shim over a default instance.
...
| D-02 | Global mutable sink registry; not thread-safe; silent failure on emit | Both (BR 2.2, DR 1.3) | **CRITICAL** | Module-level `_registered_sink` variable with no lock. `emit_to_sink` catches all exceptions as warnings. | Race conditions in async/threaded apps. Audit artifacts silently dropped — contradicts fail-closed guarantee. | Move sink to instance-scoped `AIGC` config object. Add `on_sink_failure` policy: `raise` (default), `queue`, or `log`. Deprecate `set_audit_sink()`. |
...
**Decision:** Replace module-level `_registered_sink` and all `set_*()` functions with an instance-scoped `AIGC` configuration object that holds sinks, cache settings, enforcement mode, and telemetry config.
...
2. **Instance-scoped `AIGC` object.** All config (sink, cache, mode) lives on an instance. No module-level mutation.
3. **Context-variable scoped config.** Use `contextvars.ContextVar` to scope config per async task / thread.

**Chosen Option:** Option 2 — Instance-scoped `AIGC` object.

**Tradeoffs:**
- (+) Eliminates all global mutable state.
- (+) Multiple `AIGC` instances with different configs (e.g., per-tenant) become trivial.
- (+) Testing: create instance, no global cleanup needed.
- (-) Breaking API change for `set_audit_sink()` callers.

**Implications:** `enforce_invocation()` becomes `aigc_instance.enforce(invocation)`. The module-level `enforce_invocation()` becomes a deprecated shim over a default singleton. Migration: 1-line change per call site.
...
**Implications:** `load_policy()` becomes `PolicyCompiler.compile(path) -> CompiledPolicy`. The compiled policy is immutable (frozen dataclass or `__slots__` object). Cache is instance-scoped on the `AIGC` object.
...
`AIGC` instance and execute within the pipeline at designated extension points (pre-schema, post-schema). Custom gates may FAIL
...
│                     PUBLIC API (stable)                       │
│                                                               │
│  AIGC(config)           ← configuration + enforcement entry   │
...
#### `AIGC` (Configuration + Entry Point)
...
- **Inputs:** `AIGCConfig` (sink, cache_size, cache_ttl, enforcement_mode, strict_mode, redaction_patterns, signer, observability_provider, custom_gates, policy_loader, thread_pool_size).
...
|-----|-------------|-----------|
| `set_audit_sink(sink)` | `AIGC(sink=sink)` | 1-line change |
| `get_audit_sink()` | `aigc_instance.sink` | Property access |
| `enforce_invocation(inv)` | `aigc_instance.enforce(inv)` | 1-line change |
| `enforce_invocation_async(inv)` | `aigc_instance.enforce_async(inv)` | 1-line change |
...
_default_instance: AIGC | None = None

def enforce_invocation(invocation):
    warnings.warn("Use AIGC(...).enforce() instead", DeprecationWarning, stacklevel=2)
    global _default_instance
    if _default_instance is None:
        _default_instance = AIGC()
    return _default_instance.enforce(invocation)

def set_audit_sink(sink):
    warnings.warn("Use AIGC(sink=sink) instead", DeprecationWarning, stacklevel=2)
    global _default_instance
    if _default_instance is None:
        _default_instance = AIGC(sink=sink)
    else:
        _default_instance = AIGC(sink=sink, **_default_instance._config_snapshot())
```
...
- Custom patterns configurable via `AIGC(redaction_patterns=[...])`.
...
| 6 | **Policy caching with LRU + mtime invalidation** | Fixes D-03 (CRITICAL). Performance blocker. | #1 (cache lives on AIGC instance) | Med | M | Second `enforce()` with same policy skips file I/O. Modified file triggers recompile. Configurable `cache_size`. | Unit: cache hit/miss. Unit: mtime invalidation. Benchmark: 100 enforcements, <10ms after first. | Configuration docs |
| 7 | **Sink failure mode configuration** | Completes D-02 fix. Fail-closed at persistence layer. | #1 | Low | S | `AIGC(sink=..., on_sink_failure="raise")` propagates `AuditSinkError`. Default: `"raise"`. `"log"` preserves current behavior. | Unit: each failure mode. Integration: sink that throws. | Configuration docs |
...
- Instance-scoped `AIGC` configuration (no global state)
...
- Compliance: Mock SOC 2 audit passes using only AIGC audit artifacts as evidence.
...
| Concurrency safety | 50-thread concurrent enforcement test with instance-scoped `AIGC`. No races, no shared state corruption. |
...
| Compliance evidence | Mock auditor reviews AIGC audit trail for a 10-invocation workflow and confirms: integrity, completeness, correlation, retention metadata. |

---

## 8. Threat Model Snapshot

> **Canonical reference:** [`AIGC_THREAT_MODEL.md`](AIGC_THREAT_MODEL.md) is the full threat model. This section summarizes threats in the context of roadmap mitigations; the standalone doc is authoritative for security review.
